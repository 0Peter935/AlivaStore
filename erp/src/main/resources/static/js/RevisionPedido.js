...
async function initDetallePedido() {
  ...
  document
    ...
    ?.addEventListener("click", () => onGuardarPedido(false));
...
// ======================= RENDERS =======================
function renderUltimoLog(pedido) {
  const box = document.getElementById("ultimoLogBox");
  const texto = document.getElementById("ultimoLogTexto");
  const fecha = document.getElementById("ultimoLogFecha");

  if (!pedido.logs || pedido.logs.length === 0) {
    box.classList.add("hidden");
    return;
  }

  // ordenar por fecha correcta
  const logsOrdenados = [...pedido.logs].sort((a, b) => {
    return (
      new Date(normalizarFecha(a.fechaLog)) -
      new Date(normalizarFecha(b.fechaLog))
    );
  });

  const ultimo = logsOrdenados[logsOrdenados.length - 1];

  texto.textContent = ultimo.motivoLog;

  const fechaObj = new Date(normalizarFecha(ultimo.fechaLog));

  fecha.textContent = `Registrado el: ${fechaObj.toLocaleString("es-PE", {
    hour12: false,
  })}`;
...
function normalizarFecha(f) {
  ...
  // Si ya es ISO válido, se usa tal cual
  if (f.endsWith("Z") || f.includes("+")) {
    return f;
  }

  // Si no tiene Z ni offset → agregamos Z para que sea UTC válido
  return f + "Z";
}
